 ...
 *  @discussion KQuckIM能力通过 UCSIMClientDelegate 中 didReceiveMessage: withData: 回调收到的消息。如果不设置，则无法接受聊天消息。
 *
...
/*!
 *  @discussion KQuckIM收到语音消息时首先会自动下载语音数据。在绝大部分的情况下,语音数据能够下载成功,但不排除因网络等因素带来的失败情况,这个时候开发者可以调用这个接口主动下载语音数据。一般在自动下载失败的情况下调用。
 *
 *  @brief  主动下载消息的语音数据
 *
...
/*!
 *  @discussion KQuckIM收到文件消息时不会自动下载文件数据。这个时候开发者可以调用这个接口主动下载文件数据。
 *
 *  @brief  主动下载文件数据
 *
 *  @param conversationType 会话类型
 ...
 *  @param failure   下载失败。
 */
- (void)downloadFileWithConversationType:(UCS_IM_ConversationType)conversationType
                                 targetId:(NSString *)targetId
                                messageId:(long long)messageId
                                  success:(void (^)(UCSMessage *message))success
                                  failure:(void (^)(UCSMessage *message, UCSError *error))failure;
...
/*!
 *  @discussion KQuckIM收到文件视频消息时不会自动下载视频数据。这个时候开发者可以调用这个接口主动下载视频数据。
 *
 *  @brief  主动下载视频数据
 *
 *  @param conversationType 会话类型
 ...
 *  @param failure   下载失败。
 */
- (void)downloadVideoWithConversationType:(UCS_IM_ConversationType)conversationType
                                targetId:(NSString *)targetId
                               messageId:(long long)messageId
                                 success:(void (^)(UCSMessage *message))success
                                 failure:(void (^)(UCSMessage *message, UCSError *error))failure;
...
/**
 *  @brief 清除本地保存的所有聊天数据，包括语音、图片。
 *
 *  @param block 操作结果，成功error为空，失败error不为空。
 *
 *  @warning 1、调用此接口可清除本地存储的所有IM数据,且不可恢复；
             2、调用此接口时，如有账号处于登录状态，会主动退出。
             3、清除的数据包括：聊天记录 ，账号信息等；
 */
- (void)reset:(void (^)(UCSError *error))block;
